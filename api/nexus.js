...
// api/nexus.js — Nexus v6
// Analizador de código: AST real (Babel) para JavaScript/TypeScript, reglas/regex para los otros 12 lenguajes
// api/nexus.js — Nexus v7
// JS/TS: AST real con Babel. Python: AST real con tree-sitter. Resto de 11 lenguajes: reglas/regex.
...
function universalChecks(rawCode) {
    ...
    const secretPatterns = [
        ...
        { re: /(password|contraseña|passwd|clave|secreto|secret)\s*[:=]\s*['"].{4,}['"]/i, msg: 'Contraseña/clave hardcodeada en el código' },
...
const ERROR_PATTERNS = [
    ...
    { re: /(?<!catch\s\()(?<!catch\()NullPointerException/i, lang: 'Java', causa: 'Estás llamando un método o accediendo un campo de una referencia que es null', fix: 'Agrega una validación "if (objeto != null)" antes de usarlo' },
    { re: /(?<!catch\s\()(?<!catch\()ArrayIndexOutOfBoundsException/i, lang: 'Java', causa: 'Accediste a un índice de arreglo que no existe (fuera de rango)', fix: 'Revisa que el índice esté entre 0 y length-1' },
    { re: /(?<!catch\s\()(?<!catch\()ClassNotFoundException/i, lang: 'Java', causa: 'La JVM no encuentra la clase en el classpath', fix: 'Verifica el classpath/dependencias y que el nombre del paquete sea correcto' },
    { re: /(?<!catch\s\()(?<!catch\()ConcurrentModificationException/i, lang: 'Java', causa: 'Modificaste una colección mientras la recorrías con un for-each/Iterator', fix: 'Usa un Iterator explícito con .remove(), o recorre una copia de la lista' },
    { re: /(?<!catch\s\()(?<!catch\()NumberFormatException/i, lang: 'Java', causa: 'Intentaste convertir un String a número pero no tiene formato numérico válido', fix: 'Valida el String antes de Integer.parseInt()/Double.parseDouble(), o envuélvelo en try/catch' },
    ...
    { re: /(?<!catch\s\()(?<!catch\()ClassCastException/i, lang: 'Java', causa: 'Intentaste convertir un objeto a un tipo con el que no es compatible', fix: 'Usa "instanceof" para verificar el tipo antes de castear' },
    ...
    { re: /(?<!except\s)(?<!except)KeyError:\s*\S/i, lang: 'Python', causa: 'Intentaste acceder a una clave de diccionario que no existe', fix: 'Usa dict.get("clave") en vez de dict["clave"], o valida con "in" antes' },
    ...
    { re: /(?<!except\s)(?<!except)ZeroDivisionError/i, lang: 'Python', causa: 'División entre cero', fix: 'Valida que el divisor no sea 0 antes de dividir' },
    { re: /(?<!except\s)(?<!except)RecursionError/i, lang: 'Python', causa: 'Recursión infinita o demasiado profunda', fix: 'Revisa el caso base de la función recursiva' },
...
// ============================================================
// 12.5 ANÁLISIS AST PARA JAVASCRIPT/TYPESCRIPT (Babel)
//    Reemplaza el motor regex SOLO para JS/TS — el resto de los 12 lenguajes
//    sigue usando stripStringsAndComments/checkBrackets/jsChecks sin cambios.
//    Esto elimina de raíz el bug de literales de regex (/\)/g ya no se confunde
//    con brackets de código, porque el AST sabe que es un RegExpLiteral).
// ============================================================
function reconocerProposito(path, funciones) {
    ...
    if (params.includes('req') && params.includes('res') && params.includes('next')) {
        funciones.push({ line, tipo: 'Middleware de Express (req, res, next)' }); return;
    }
    if (params.includes('req') && params.includes('res')) {
        funciones.push({ line, tipo: 'Route handler de Express (req, res)' }); return;
    }
    if (params.includes('resolve') && params.includes('reject')) {
        funciones.push({ line, tipo: 'Promise executor (resolve, reject)' }); return;
    }
    const parent = path.parentPath?.node;
    if (parent?.type === 'CallExpression' && parent.callee?.property?.name === 'addEventListener') {
        funciones.push({ line, tipo: 'Event listener (addEventListener)' }); return;
    }
    ...
    if (fnName && /^[A-Z]/.test(fnName)) {
        ...
        for (const stmt of body) {
            if (stmt.type === 'ReturnStatement' && stmt.argument &&
                ...
                funciones.push({ line, tipo: `Componente React "${fnName}"` });
                break;
            }
        }
    }
...
function analyzeJavaScriptAst(code) {
    let ast;
    try {
        ast = parse(code, { sourceType: 'unambiguous', errorRecovery: false, plugins: ['jsx', 'typescript'] });
    } catch (e) {
        return { syntaxError: { line: e.loc?.line || 0, message: e.message.split('\n')[0] } };
    }

    const issues = [];
    const funciones = [];
    let maxDepth = 0, maxDepthLine = 0;
    let decisionCount = 0;
    let hasThen = false, hasCatch = false, hasAwait = false, hasTry = false, hasMathRandom = false;

    const BLOCK_TYPES = new Set(['IfStatement', 'ForStatement', 'ForInStatement', 'ForOfStatement',
        'WhileStatement', 'DoWhileStatement', 'SwitchStatement', 'TryStatement']);
    const DECISION_TYPES = new Set(['IfStatement', 'ForStatement', 'ForInStatement', 'ForOfStatement',
        'WhileStatement', 'DoWhileStatement', 'SwitchCase', 'CatchClause', 'LogicalExpression', 'ConditionalExpression']);

    function checkFnSize(path) {
        const start = path.node.loc?.start.line;
        const end = path.node.loc?.end.line;
        if (start && end && (end - start) > 50) {
            issues.push({ line: start, severity: 'info', message: `Función de ${end - start} líneas — considera dividirla en funciones más pequeñas` });
        }
        reconocerProposito(path, funciones);
    }

    traverse(ast, {
        enter(path) {
            if (BLOCK_TYPES.has(path.node.type)) {
                let depth = 1, p = path.parentPath;
                while (p) { if (BLOCK_TYPES.has(p.node.type)) depth++; p = p.parentPath; }
                if (depth > maxDepth) { maxDepth = depth; maxDepthLine = path.node.loc.start.line; }
            }
            if (DECISION_TYPES.has(path.node.type)) decisionCount++;
        },
        CallExpression(path) {
            const callee = path.node.callee;
            const line = path.node.loc?.start.line || 0;
            const name = callee.type === 'Identifier' ? callee.name : (callee.property && callee.property.name);
            if (name === 'eval' && callee.type === 'Identifier') issues.push({ line, severity: 'critico', message: 'eval() es un riesgo de seguridad grave — evita ejecutar strings como código' });
            if (name === 'write' && callee.object?.name === 'document') issues.push({ line, severity: 'warning', message: 'document.write() es mala práctica y riesgo de XSS' });
            if ((name === 'log' || name === 'debug') && callee.object?.name === 'console') issues.push({ line, severity: 'info', message: `console.${name} de depuración olvidado` });
            if (name === 'then' && callee.type === 'MemberExpression') hasThen = true;
            if (name === 'catch' && callee.type === 'MemberExpression') hasCatch = true;
            if (name === 'random' && callee.object?.name === 'Math') hasMathRandom = true;
        },
        AwaitExpression() { hasAwait = true; },
        TryStatement() { hasTry = true; },
        DebuggerStatement(path) { issues.push({ line: path.node.loc.start.line, severity: 'warning', message: 'Sentencia "debugger" olvidada' }); },
        CatchClause(path) { if (path.node.body.body.length === 0) issues.push({ line: path.node.loc.start.line, severity: 'warning', message: 'Bloque catch vacío: está silenciando el error' }); },
        VariableDeclaration(path) { if (path.node.kind === 'var') issues.push({ line: path.node.loc.start.line, severity: 'info', message: 'Usa "let" o "const" en vez de "var"' }); },
        BinaryExpression(path) {
            if (path.node.operator === '==') issues.push({ line: path.node.loc.start.line, severity: 'warning', message: 'Usa "===" en vez de "==" (comparación estricta)' });
            if (path.node.operator === '!=') issues.push({ line: path.node.loc.start.line, severity: 'warning', message: 'Usa "!==" en vez de "!=" (comparación estricta)' });
        },
        AssignmentExpression(path) {
            if (path.node.left.type === 'MemberExpression' && path.node.left.property?.name === 'innerHTML' && path.node.right.type !== 'StringLiteral') {
                issues.push({ line: path.node.loc.start.line, severity: 'warning', message: 'innerHTML con una variable — riesgo de XSS. Usa textContent o sanitiza' });
            }
        },
        IfStatement(path) {
            if (path.node.test.type === 'AssignmentExpression' && path.node.test.operator === '=') {
                issues.push({ line: path.node.loc.start.line, severity: 'error', message: 'Posible bug: asignación "=" dentro de una condición (¿querías "=="?)' });
            }
        },
        FunctionDeclaration: checkFnSize,
        FunctionExpression: checkFnSize,
        ArrowFunctionExpression: checkFnSize,
    });

    if (hasThen && !hasCatch) issues.push({ line: 0, severity: 'warning', message: 'Promise con .then() sin .catch() — errores no controlados' });
    if (hasAwait && !hasTry && !hasCatch) issues.push({ line: 0, severity: 'warning', message: 'Uso de "await" sin try/catch ni .catch() — errores no controlados' });
    if (hasMathRandom && /(token|password|contrase|secret|session)/i.test(code)) issues.push({ line: 0, severity: 'critico', message: 'Math.random() no es criptográficamente seguro — no lo uses para tokens/contraseñas/sesiones. Usa crypto.randomBytes()' });
    if (maxDepth > 4) issues.push({ line: maxDepthLine, severity: 'info', message: `Anidación profunda (nivel ${maxDepth}) — considera extraer funciones o usar "early return"` });
    const totalLines = code.split('\n').length;
    if (totalLines > 15 && decisionCount / totalLines > 0.35) issues.push({ line: 0, severity: 'info', message: `Complejidad ciclomática aproximada alta (${decisionCount} puntos de decisión en ${totalLines} líneas) — considera simplificar` });

    return { issues, funciones };
}

// ============================================================
// 12.6 ANÁLISIS AST PARA PYTHON (tree-sitter)
//    Igual que con JS/Babel: reemplaza el motor regex SOLO para Python.
//    Carga perezosa (solo si se detecta código Python) y una sola vez por
//    invocación fría de la función — no en cada análisis.
// ============================================================
let tsParserInitialized = false;
let pythonLanguage = null;

async function getPythonParser() {
    if (!tsParserInitialized) {
        await TSParser.init();
        tsParserInitialized = true;
    }
    if (!pythonLanguage) {
        const wasmPath = path.join(process.cwd(), 'node_modules/tree-sitter-wasms/out/tree-sitter-python.wasm');
        const bytes = fs.readFileSync(wasmPath);
        pythonLanguage = await TSLanguage.load(new Uint8Array(bytes));
    }
...
async function analyzePythonAst(code) {
    ...
    try {
        ...
        return null; // el runtime no pudo cargar el .wasm -> se hace fallback a regex
    }
    ...
    if (tree.rootNode.hasError) {
        // Buscar el primer nodo de error para dar una línea aproximada
        let errLine = 0;
        (function findErr(node) {
            if (errLine) return;
            if (node.type === 'ERROR' || node.isMissing) { errLine = node.startPosition.row + 1; return; }
            for (const c of node.namedChildren) findErr(c);
        })(tree.rootNode);
        issues.push({ line: errLine, severity: 'error', message: 'Error de sintaxis detectado por el parser' });
        return { issues, funciones: [] };
    }
    ...
    let maxDepth = 0, maxDepthLine = 0;
    let decisionCount = 0;
    const BLOCK_TYPES = new Set(['if_statement', 'for_statement', 'while_statement', 'try_statement', 'with_statement']);
    const DECISION_TYPES = new Set(['if_statement', 'for_statement', 'while_statement', 'except_clause', 'boolean_operator', 'conditional_expression']);

    function walk(node, depth) {
        if (BLOCK_TYPES.has(node.type)) {
            depth++;
            if (depth > maxDepth) { maxDepth = depth; maxDepthLine = node.startPosition.row + 1; }
        }
        if (DECISION_TYPES.has(node.type)) decisionCount++;

        if (node.type === 'except_clause' && node.namedChildCount === 1) {
            issues.push({ line: node.startPosition.row + 1, severity: 'warning', message: 'except sin tipo — captura todo, incluso errores inesperados' });
        }
        if (node.type === 'default_parameter') {
            const valueNode = node.namedChildren[1];
            if (valueNode && (valueNode.type === 'list' || valueNode.type === 'dictionary')) {
                issues.push({ line: node.startPosition.row + 1, severity: 'error', message: 'Argumento por defecto mutable ([] o {}) — bug clásico de Python' });
            }
        }
        if (node.type === 'comparison_operator') {
            if (/==\s*None\b/.test(node.text)) issues.push({ line: node.startPosition.row + 1, severity: 'info', message: 'Usa "is None" en vez de "== None"' });
            if (/==\s*(True|False)\b/.test(node.text)) issues.push({ line: node.startPosition.row + 1, severity: 'info', message: 'Comparación redundante con True/False, usa la variable directo' });
        }
        if (node.type === 'call') {
            const fn = node.namedChildren[0];
            const fnName = fn?.type === 'attribute' ? fn.namedChildren[1]?.text : fn?.text;
            if (fnName === 'eval') issues.push({ line: node.startPosition.row + 1, severity: 'critico', message: 'eval() es un riesgo de seguridad grave' });
            if (fnName === 'exec') issues.push({ line: node.startPosition.row + 1, severity: 'critico', message: 'exec() ejecuta código arbitrario — riesgo de seguridad' });
            if (fnName === 'loads' && fn?.namedChildren[0]?.text === 'pickle') {
                issues.push({ line: node.startPosition.row + 1, severity: 'critico', message: 'pickle.loads() sobre datos no confiables permite ejecución de código arbitrario' });
            }
            if (fnName === 'print') issues.push({ line: node.startPosition.row + 1, severity: 'info', message: 'print() de depuración' });
        }
        if (node.type === 'keyword_argument' && node.namedChildren[0]?.text === 'shell' && node.namedChildren[1]?.text === 'True') {
            issues.push({ line: node.startPosition.row + 1, severity: 'critico', message: 'shell=True con input externo es riesgo de inyección de comandos' });
        }
        if (node.type === 'function_definition') {
            const start = node.startPosition.row + 1, end = node.endPosition.row + 1;
            if (end - start > 50) issues.push({ line: start, severity: 'info', message: `Función de ${end - start} líneas — considera dividirla en funciones más pequeñas` });
        }

        for (const child of node.namedChildren) walk(child, depth);
    }
    walk(tree.rootNode, 0);

    if (maxDepth > 4) issues.push({ line: maxDepthLine, severity: 'info', message: `Anidación profunda (nivel ${maxDepth}) — considera extraer funciones o usar "early return"` });
    const totalLines = code.split('\n').length;
    if (totalLines > 15 && decisionCount / totalLines > 0.35) {
        issues.push({ line: 0, severity: 'info', message: `Complejidad ciclomática aproximada alta (${decisionCount} puntos de decisión en ${totalLines} líneas) — considera simplificar` });
    }
...
async function analyzeCode(code) {
    ...
    if (lang === 'javascript' || lang === 'typescript') {
        const ast = analyzeJavaScriptAst(code);
        if (ast.syntaxError) {
            issues.push({ line: ast.syntaxError.line, severity: 'error', message: `Error de sintaxis: ${ast.syntaxError.message}` });
        } else {
            issues.push(...ast.issues);
            funciones = ast.funciones;
        }
    } else if (lang === 'python') {
        const ast = await analyzePythonAst(code);
        if (ast) {
            issues.push(...ast.issues);
            funciones = ast.funciones;
        } else {
            // tree-sitter no pudo cargar (ej. .wasm no empaquetado) -> respaldo con motor regex
            const stripped = stripStringsAndComments(code, lang);
            issues.push(...pyChecks(code.split('\n'), stripped.split('\n')));
    ...
    return { lang, issues, funciones };
}

async function buildAnalysisResponse(code) {
    const lineCount = code.split('\n').length;
    const { lang, issues, funciones } = await analyzeCode(code);
    const langName = LANG_NAMES[lang] || 'código';
    const seccionFunciones = (funciones && funciones.length > 0)
        ? '\n\n🏗️ Reconocí: ' + funciones.map(f => `${f.tipo} (línea ${f.line})`).join(', ')
        : '';
    ...
    if (issues.length === 0) {
        return {
            ...
            code_snippet: (funciones && funciones.length > 0) ? funciones.map(f => `🏗️ ${f.tipo} (línea ${f.line})`).join('\n') : '',
            lang, issues,
        };
    }
